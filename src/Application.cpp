...
void Application::start(void){
	mResourcesCfg = INITIAL_RESOURCE_PATH;
	mPluginsCfg = PLUGIN_PATH;
...
void Application::createScene(void){
	...
	//attach to nodes
	node1->attachObject(testEnt1);
	node2->attachObject(testEnt2);

	//move nodes
	node2->translate(Ogre::Vector3(5, 0, 0));
	...
	//Ogre::Entity* lightSphere = mSceneMgr->createEntity("MyEntity1","sphere.mesh");
	//node->attachObject(lightSphere);
	//Ogre::SceneNode* node3 = node->createChildSceneNode("node3");
	//node3->setScale(0.1f,0.1f,0.1f);
	//node3->setPosition(0,10,0);
